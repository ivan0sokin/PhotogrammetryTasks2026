...
cv::Mat buildTransform(
    ...
    if (parent[imgIdx] == -1) {
        return Hs[imgIdx] = cv::Mat::eye(3, 3, CV_64FC1);
    }

    return Hs[imgIdx] = homography_builder(imgs[imgIdx], imgs[parent[imgIdx]]) * buildTransform(
        parent[imgIdx],
        used,
        Hs,
        imgs,
                            ...
                            std::function<cv::Mat(const cv::Mat &, const cv::Mat &)> &homography_builder)
{
    ...
    std::vector<cv::Mat> Hs(n_images);
    {
        ...
        for (size_t imgIdx = 0; imgIdx < n_images; ++imgIdx) {
            buildTransform(
                imgIdx,
                used,
                Hs,
